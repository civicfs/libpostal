...
This is a native Go implementation of libpostal for parsing and normalizing international street addresses using statistical NLP and open data.

> **Note**: This repository contains a Go refactor of the original [libpostal C library](https://github.com/openvenues/libpostal). The C implementation has been replaced with idiomatic Go code while maintaining the same core functionality.

**Original C library**: For the original C implementation and its language bindings (Python, Ruby, Java, PHP, NodeJS), see the [original libpostal repository](https://github.com/openvenues/libpostal).

**Background**: libpostal is designed to understand location-based strings in every language, everywhere. For a comprehensive overview of the research behind libpostal, check out the introductory blog posts:
